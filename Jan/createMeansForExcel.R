...
for(x in 1:nrow(inputMachine)){
  ...
  bDay <- inputDate[which(inputDate[,"ID"] == gsub(" ","",ID)),"Bday"]                                    # Date of birth
  mDay <- inputMachine[x, "TimeDate"]                                                                     # Date of measurement
...
inputMachine <- cbind(inputMachine, Diff = daysDifferences)                                               # Add the differences between measurement and birth
...
outPut <- matrix(NA, length(uniqueIDs), length(columnnames))                                              # Output matrix
colnames(outPut) <- columnnames                                                                           # Columns are: Lean_X, Fat_X, where X = time difference
rownames(outPut) <- uniqueIDs                                                                             # Rows are the mouse IDs
...
for(ID in uniqueIDs){
  ...
  for(difference in differences){
    ...
    fatMean <- mean(as.numeric(gsub(" ","", inputMachine[toMeanIDs,"Fat"])))                              # Calculate the mean Fat
    leanMean <- mean(as.numeric(gsub(" ","", inputMachine[toMeanIDs,"Lean"])))                            # Calculate the mean Lean
