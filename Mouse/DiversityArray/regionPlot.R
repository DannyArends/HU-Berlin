...
snpOUT86012  <- read.table("Analysis/Diabetes/BFMI861-S1andBFMI860-12vsALL_SNPs.txt", sep="\t", header=TRUE, colClasses=c("character"))
regions86012 <- read.table("Analysis/Diabetes/BFMI861-S1andBFMI860-12vsALL_Regions.txt", sep="\t", header=TRUE)

snpOUT861S1  <- read.table("Analysis/Diabetes/BFMI861-S1vsALL_SNPs.txt", sep="\t", header=TRUE, colClasses=c("character"))
regions861S1 <- read.table("Analysis/Diabetes/BFMI861-S1vsALL_Regions.txt", sep="\t", header=TRUE)


plotregions <- function(snpOUT, regions, name="Analysis/Figures/BFMI861-S1andBFMI860-12_Region"){
  asSNPs <- matrix(NA,nrow(snpOUT),length(9:ncol(snpOUT)))
  for(x in 1:nrow(snpOUT)){
    for(i in 9:ncol(snpOUT)){
      if(is.na(snpOUT[x,i])){
        asSNPs[x,(i-8)] <- NA
      }else{
  ...
  colnames(numgeno) <- colnames(snpOUT[9:ncol(snpOUT)])

  for(x in 1:nrow(regions)){
    png(paste0(name, x, ".png"), width=2000, height = 800)
...
plotregions(snpOUT86012, regions86012)
plotregions(snpOUT861S1, regions861S1, "Analysis/Figures/BFMI861-S1_Region")