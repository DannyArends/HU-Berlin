...
sampledescription      <- "/home/b/bebdanny/experiment/sampledescription.txt"
reference              <- "/gfs1/work/bebdanny/genomes/mm10/Mus_musculus.GRCm38.dna"
...
transcriptome.index    <- "/gfs1/work/bebdanny/genomes/mm10/tophat2/Mus_musculus.GRCm38.genes"
reference.snps         <- "/gfs1/work/bebdanny/genomes/mm10/mgp.v5.merged.snps_all.dbSNP142.vcf.gz"          # wget ftp://ftp-mouse.sanger.ac.uk/current_snps/mgp.v5.merged.snps_all.dbSNP142.vcf.gz
reference.indels       <- "/gfs1/work/bebdanny/genomes/mm10/mgp.v5.merged.indels.dbSNP142.normed.vcf.gz"     # wget ftp://ftp-mouse.sanger.ac.uk/current_snps/mgp.v5.merged.indels.dbSNP142.normed.vcf.gz
reference.gtf          <- "/gfs1/work/bebdanny/genomes/mm10/Mus_musculus.GRCm38.81.gtf"                      # wget ftp://ftp.ensembl.org/pub/release-81/gtf/mus_musculus/Mus_musculus.GRCm38.81.gtf.gz
indels.intervals       <- "/gfs1/work/bebdanny/genomes/mm10/output.indels.intervals"

picarddict.exec        <- "java -Xmx4g -jar /home/b/bebdanny/bin/picard-tools-1.99/CreateSequenceDictionary.jar"
picardrmdup.exec       <- "java -Xmx4g -jar /home/b/bebdanny/bin/picard-tools-1.99/MarkDuplicates.jar"
picardrg.exec          <- "java -Xmx4g -jar /home/b/bebdanny/bin/picard-tools-1.99/AddOrReplaceReadGroups.jar"
picardreo.exec         <- "java -Xmx4g -jar /home/b/bebdanny/bin/picard-tools-1.99/ReorderSam.jar"
gatk.exec              <- "~/tools/jre1.7.0_80/bin/java -Xmx64g -jar /home/b/bebdanny/bin/GATK/GenomeAnalysisTK.jar"
b2build.exec           <- "/home/b/bebdanny/bin/bowtie2-2.2.6/bowtie2-build"
bowtie2.exec           <- "/home/b/bebdanny/bin/bowtie2-2.2.6/bowtie2"
samtools.exec          <- "/home/b/bebdanny/bin/samtools-1.2/samtools"
tabix.exec             <- "/home/b/bebdanny/bin/tabix-0.2.6/tabix"
tophat2.exec           <- "/home/b/bebdanny/bin/tophat-2.1.0/tophat2"
trimmomatic.exec       <- "java -Xmx8g -jar /home/b/bebdanny/bin/Trimmomatic-0.32/trimmomatic-0.32.jar"
trimmomatic.adapter    <- "/home/b/bebdanny/bin/Trimmomatic-0.32/adapters/TruSeq3-PE-2.fa"
...
execute(paste0(tophat2.exec, " -G ", reference.gtf, " --transcriptome-index=", transcriptome.index, " ", reference.bt2idx), transcriptome.index)
...
inS       <- grepl(substr(fname, 1, 4), paste(samples[,"Lib_id"],samples[,"TagLane"],sep="_"))
...
trimmomatic.opts   <- paste0("ILLUMINACLIP:", trimmomatic.adapter ,":2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36")
...
tophat.options  <- paste0("--prefilter-multihits --rg-id ", readgroupID, " --rg-sample ", fname, " --rg-library RNA-seq --rg-platform Illumina --b2-sensitive --read-gap-length 12 --read-mismatches 2 --read-edit-dist 13 --max-insertion-length 20 --max-deletion-length 30 --num-threads 18",collapse="")
tophat.log      <- paste0(filebase, ".tophat2.log")
execute(paste0(tophat2.exec, " -o ", alignment.file, " --transcriptome-index ", transcriptome.index, " ", tophat.options, " ", reference.bt2idx, " ", trimmomatic.files[3], " ", trimmomatic.files[5], ",", trimmomatic.files[4], ",", trimmomatic.files[6], " > ", tophat.log, collapse=""), alignment.file)
...
execute(paste0(samtools.exec, " sort -@ 18 -m 2G ",alignment.file, " ", samtools.file), samtools.bamfile)
...
execute(paste0(gatk.exec, " -T IndelRealigner -R ", reference.fa, " -targetIntervals ", indels.intervals , " -I ", picard.rgfile.reordered, " -o ", gatk.realigned, " -known ", reference.indels, " -U ALLOW_N_CIGAR_READS"), gatk.realigned)

# Base Recalibration
gatk.cov1 <- paste0(filebase, ".1.covariates")
execute(paste0(gatk.exec, " -nct 18 -T BaseRecalibrator -R ", reference.fa," -knownSites ", reference.snps, " -I ", gatk.realigned, " -o ", gatk.cov1," -U ALLOW_N_CIGAR_READS"), gatk.cov1)
gatk.recal <- paste0(filebase, ".aln.sort.rmdup.rg.realigned.recal.bam")
execute(paste0(gatk.exec, " -nct 18 -T PrintReads -R ", reference.fa," -I ",gatk.realigned," -BQSR ", gatk.cov1," -U ALLOW_N_CIGAR_READS -o ", gatk.recal), gatk.recal)
gatk.cov2 <- paste0(filebase, ".2.covariates")
execute(paste0(gatk.exec, " -nct 18 -T BaseRecalibrator -R ", reference.fa," -knownSites ", reference.snps, "  -I ",gatk.recal," -o ", gatk.cov2," -U ALLOW_N_CIGAR_READS"), gatk.cov2)
