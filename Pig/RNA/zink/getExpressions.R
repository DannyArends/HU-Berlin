...
chrominfo     <- read.table("GTF/PigChrInfo.txt", sep="\t", header=TRUE, colClasses=c("character","integer","logical"))

# Download GTF file from: ftp://ftp.ensembl.org/pub/release-76/gtf/mus_musculus/
pig         <- makeTxDbFromGFF("GTF/Sus_scrofa.Sscrofa10.2.84.short.gtf", format = "gtf", 
                                         organism="Sus scrofa", chrominfo=chrominfo, dataSource="ftp://ftp.ensembl.org/pub/release-84/gtf/sus_scrofa/")
exonsByGene   <- exonsBy(pig, by = "gene")
infiles       <- list.files(path = "./", pattern="recal.bam$", full=TRUE)
...
write.table(assay(se), file="RawReads.txt", sep="\t")                              # Save the raw reads to a file
...
write.table(RPKM, file="RPKMnorm.txt", sep="\t")                                  # Write the normalized RPKM values to file
