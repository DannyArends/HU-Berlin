...
points(cor(pheM,use="pair")["urineVolume",], col="blue", pch=19,cex=2)
points(cor(pheF,use="pair")["urineVolume",], col="hotpink", pch=18,cex=2)
axis(1, at = 1:length(phenotypes), phenotypes, las=2)

# Minimal model (include sex as covariate)
anova(lm(urineVolume ~ sex, data = phe))
# Standard covars
anova(lm(urineVolume ~ sex + breedingCage, data = phe))
anova(lm(urineVolume ~ sex + ageAtDosing, data = phe))
anova(lm(urineVolume ~ sex + ageAtUC, data = phe))
# H2O related
anova(lm(urineVolume ~ sex + waterConsumed, data = phe))
anova(lm(urineVolume ~ sex + totalWaterInitial, data = phe))
# Weights & MRI
anova(lm(urineVolume ~ sex + weightBeforeDiet, data = phe))
anova(lm(urineVolume ~ sex + weightInitialMRI, data = phe))
anova(lm(urineVolume ~ sex + weightFinalMRI, data = phe))
anova(lm(urineVolume ~ sex + fatInitial, data = phe))
anova(lm(urineVolume ~ sex + leanInitial, data = phe))
anova(lm(urineVolume ~ sex + fatGain, data = phe))
anova(lm(urineVolume ~ sex + weightGain, data = phe))

# Building a minimal model for QTL mapping of BLL
anova(lm(urineVolume ~ sex + ageAtDosing + ageAtUC + waterConsumed + weightBeforeDiet + weightInitialMRI+ weightFinalMRI+ breedingCage + weightInitialMRI + fatInitial + leanInitial + totalWaterInitial + fatGain, data = phe))
anova(lm(urineVolume ~ sex + ageAtDosing + waterConsumed + weightInitialMRI + weightFinalMRI+ fatInitial + fatGain, data = phe))
anova(lm(urineVolume ~ sex + ageAtDosing + waterConsumed + weightFinalMRI+ fatInitial + fatGain, data = phe))
anova(lm(urineVolume ~ sex + waterConsumed + ageAtDosing + fatInitial + fatGain, data = phe)) # All factors highly significant
...
models.raw <- apply(gts,1,function(gt){ return(lm(urineVolume ~ sex + gt, data = phe)) })
models.adjusted <- apply(gts,1,function(gt){ return(lm(urineVolume ~ sex + ageAtDosing + waterConsumed + fatInitial + fatGain + gt, data = phe)) })
models.interaction <- apply(gts, 1, function(gt){ return(lm(urineVolume ~ sex + ageAtDosing + waterConsumed + fatInitial + fatGain + gt + sex:gt, data = phe)) })

# Male / Female only models
models.males <- apply(gts[mHas, rownames(pheM)], 1, function(gt){ return(lm(urineVolume ~ ageAtDosing + waterConsumed + fatInitial + fatGain + gt, data = pheM)) })
models.females <- apply(gts[fHas, rownames(pheF)], 1, function(gt){ return(lm(urineVolume ~ ageAtDosing + waterConsumed + fatInitial + fatGain + gt, data = pheF)) })
...
plot(c(0, max(map[,"cumPos"],na.rm=TRUE)), c(0, 15), main = "Raw: urineVolume ~ sex + gt", t='n', xlab="Chromosome", ylab="-log10(P)", xaxt='n', las=2)
for(chr in chroms){ 
  idx <- which(map[,1] == chr)
...
plot(c(0, max(map[,"cumPos"],na.rm=TRUE)), c(0, 15), main = "Combined: urineVolume ~ sex + ageAtDosing + waterConsumed + fatInitial + fatGain + gt", t='n', xlab="Chromosome", ylab="-log10(P)", xaxt='n', las=2)
for(chr in chroms){ 
  idx <- which(map[,1] == chr)
...
plot(c(0, max(map[,"cumPos"],na.rm=TRUE)), c(0, 15), main = "Interaction: urineVolume ~ sex + ageAtDosing + waterConsumed + fatInitial + fatGain + gt + gt:sex", t='n', xlab="Chromosome", ylab="-log10(P)", xaxt='n', las=2)
for(chr in chroms){ 
  idx <- which(map[,1] == chr)
...
plot(c(0, max(map[,"cumPos"],na.rm=TRUE)), c(0, 15), main = "Males: urineVolume ~ ageAtDosing + waterConsumed + fatInitial + fatGain + gt", t='n', xlab="Chromosome", ylab="-log10(P)", xaxt='n', las=2)
for(chr in chroms){ 
  idx <- which(map[,1] == chr)
...
plot(c(0, max(map[,"cumPos"],na.rm=TRUE)), c(0, 15), main = "Females: urineVolume ~ ageAtDosing + waterConsumed + fatInitial + fatGain + gt", t='n', xlab="Chromosome", ylab="-log10(P)", xaxt='n', las=2)
for(chr in chroms){ 
  idx <- which(map[,1] == chr)
